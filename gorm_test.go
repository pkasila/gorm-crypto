...
func TestAESFallbacks(t *testing.T) {
	aes, err := algorithms.NewAES256GCM([]byte("passphrasewhichneedstobe32cytes!"))
	if err != nil {
		panic(err)
	}

	// New main algorithm
	aes2, err := algorithms.NewAES256GCM([]byte("passphrasewhichneedstobe32bytes!"))
	if err != nil {
		t.Fatalf("Cannot find TestModel entity: %s\n", err.Error())
	}

	// Initialize library with fallbacks
	InitWithFallbacks([]algorithms.Algorithm{aes2, aes}, []serialization.Serializer{serialization.NewJSON()})
